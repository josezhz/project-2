...
export default class UpdateTeam extends React.Component {
    ...
    componentDidMount = () => {
        ...
        this.setState({
            ...
            teamMembersBeingAdded: t.team_composition,
            bossesBeingAdded: t.bosses.map(b => b.$oid),
            rotationGuideStepsBeingAdded: t.rotation_guide,
            notesBeingAdded: t.notes
        })
    }
                            ...
                            <div>
                                <button className="w-100 btn btn-primary rounded-pill py-0 mb-2"
                                    onClick={async () => {
                                        ...
                                        await axios.put(this.props.BASE_URI + "teams/" + this.state.teamId, {
                                            modifiedTeam
                                        });
                                        await this.props.refreshTeams();
                                        this.props.updateTeamBeingUpdated(null);
                                    }}
                                    disabled={!(
                                        this.state.teamName
